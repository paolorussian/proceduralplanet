...
public class Chunk : MonoBehaviour {
	...
	void Update () {
        ...
        float dist = Vector3.Distance(gameObject.transform.TransformPoint(Camera.main.transform.parent.position), worldPt);
        ...
        } else if (lodLevel>0 &&  dist > diagonal * 3 && mesh != null  && !parent.isRestoringParent) {
    ...
    /*
    public CellEdge getOrAdd(CellEdge c) {


        if (edgeDictionary.ContainsKey(c.name)) {

            return edgeDictionary[c.name];
        } else {
            edgeDictionary.Add(c.name, c);
            return edgeDictionary[c.name];
        }
